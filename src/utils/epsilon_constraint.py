...
from pyomo.environ import (Model, Objective, SolverFactory,
                                                value, Constraint, Param,
                                                Var, Set, NonNegativeReals,
                                                maximize, SolverStatus)
...
class Solution():
    ...
    def add_decision_var_value(self, decision_name: str,
                                                   value):
        self._decision_vars[decision_name] = value
    
    def add_objective_value(self, objective_name: str,
                                             value):
...
class EpsilonConstraintAugment():
    ...
    @staticmethod
    def __get_interval_points(lowerbound: float, upperbound: float,
                                               num_points: int = 4) -> List[float]:
        ...
        Args:
            ...
            num_points (int, optional):  num of equidistant points - 1. Defaults to 4.
    ...
    @staticmethod
    def __solve_instance(model: Model):
    ...
    def __iterate_payoff_table(self, payoff_table: dict, **kwargs):
        ...
        for grid_point in grid_point_list:
            if not payoff_table:
                ...
                efficient_solution = self.__iterate_payoff_table(self.model, payoff_table, obj_name=grid_point)
    ...
    def __get_solutions(model: Model):
        ...
        for v in model.component_data_objects(Var, active=True):
            if v.name not in s_var_list:
                solution.add_decision_var_value(v.name,
                                                                      value(v))
        for v in model.component_data_objects(Objective):
            if v.name != 'F':
                solution.add_objective_value(v.name,
                                                                 value(v))
        return solution
    ...
    def __solve_epsilon_augmenton(self, grid_points: dict):
        ...
        self.__epsilon_constraints(self.model,
                                   grid_points)
        result = self.__solve_instance(self.model)
    ...
    def execute(self) -> list: 
        ...
        for obj in objectives:
            obj.activate()
            self.__solve_instance(self.model)
            obj.deactivate()
            payoff_table[obj.name].append(value(obj))
            
            setattr(self.model,
                        f'c_aux_{obj.name}',
                        Param(initialize=payoff_table[obj.name][-1],
                                                                               mutable=False))
            setattr(self.model, 
                        f'c_{obj.name}',
                        Constraint(expr=  obj >= getattr(self.model, f'c_aux_{obj.name}')))
            ...
            for obj1 in objectives:
                if obj.name != obj1.name:
                    ...
                    setattr(self.model, 
                            f'c_{obj1.name}',
                            Constraint(expr=  obj >= getattr(self.model, f'c_aux_{obj1.name}')))
            ...
            for obj in objectives:
                ...
                self.model.del_component(getattr(self.model, f'c_aux_{obj.name}'))
        
        for obj in objectives:
            payoff_table[obj.name] = [min(payoff_table[obj.name]),
                                                    max(payoff_table[obj.name])]
        
        payoff_table.pop(objectives[0].name)
        
        objective_points = {k:self.__get_interval_points(v[0], v[1])
        ...
        self.model.F = Objective(rule= self.__epsilon_objective(self.model, objectives_name[0]),
                                                                                                    sense=maximize)
    ...
    def get_efficient_solutions(self, solution_list: List[Solution]):
        ...
        while next_point_index < len(costs):
            nondominated_point_mask = [any(costs[j][i] > costs[next_point_index][i] for i in range(sol_len))\
                                                                for j in range(len(costs))]
            nondominated_point_mask[next_point_index] = True
            is_efficient = [is_efficient[j] for j in range(len(costs)) if nondominated_point_mask[j]]
